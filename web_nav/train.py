...
def train_eval(
    ...
    with tf.compat.v2.summary.record_if(
        ...
        lock = manager.Lock()
        global_vocab = (
            rl_perf.domains.web_nav.CoDE.vocabulary_node.LockedVocabulary(multiprocessing_lock=lock)
        )
        ...
        # remove train_eval.* from the gin config since it is not defined in the parallel env constructors
        gin_config = gin.config_str()
        gin_config = remove_config_lines(gin_config, 'train_eval.')
        gin.clear_config()
        gin.parse_config(gin_config)

        parallel_py_env = tf_agents.environments.ParallelPyEnvironment(envs, start_serially=True)
        batched_tf_env = tf_py_environment.TFPyEnvironment(parallel_py_env)
        ...
        with tf.name_scope('DQNAgent'):
            q_net = DQNLSTM(
                ...
                else batched_tf_env.pyenv.envs[0].env.local_vocab.max_vocabulary_size,
        ...
        for _ in range(num_iterations):
            ...
            if global_step.numpy() % summary_interval == 0:
                ...
                train_df = pd.concat(
                    [train_df, pd.DataFrame([results])], ignore_index=True
                )
                train_df.to_csv(
                    os.path.join(train_dir, 'train_summary.csv'), index=False
                )
                del train_df
            ...
            if global_step.numpy() % eval_interval == 0:
                ...
                eval_df = pd.concat(
                    [eval_df, pd.DataFrame([results])], ignore_index=True
                )
                eval_df.to_csv(os.path.join(eval_dir, 'eval_summary.csv'), index=False)
                del eval_df
