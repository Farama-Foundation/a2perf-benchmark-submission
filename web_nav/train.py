...
def train_mp(_):
    ...
    eval_interval = int(os.environ.get('EVAL_INTERVAL', 100000))
    train_checkpoint_interval = int(os.environ.get('TRAIN_CHECKPOINT_INTERVAL', 100000))
    policy_checkpoint_interval = int(os.environ.get('POLICY_CHECKPOINT_INTERVAL', 100000))
    rb_checkpoint_interval = int(os.environ.get('RB_CHECKPOINT_INTERVAL', 20000))
    log_interval = int(os.environ.get('LOG_INTERVAL', TIMESTEPS_PER_ACTORBATCH))
    summary_interval = int(os.environ.get('SUMMARY_INTERVAL', TIMESTEPS_PER_ACTORBATCH))
    learning_rate = float(os.environ.get('LEARNING_RATE', 1e-4))
    timesteps_per_actorbatch = max(1, TIMESTEPS_PER_ACTORBATCH // env_batch_size)
    num_iterations = max(1, total_env_steps // timesteps_per_actorbatch)
    ...
    train_eval(
        ...
        env_args={'designs': [{'number_of_pages': 1, 'action': [], 'action_page': [], }]}
    )
