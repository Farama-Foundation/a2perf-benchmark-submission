...
def train_eval(
  ...
  with tf.compat.v2.summary.record_if(
    ...
    global_vocab = (
        rl_perf.domains.web_nav.CoDE.vocabulary_node.LockedVocabulary()
    )

    def create_env_fn(create_env_seed):

      # Have to parse gin config again due to multiprocessing
      ...
      return suite_gym.load(
          ...
          gym_kwargs={
              ...
              'seed': create_env_seed,
              'global_vocabulary': global_vocab,
          },
      )

    parallel_tf_env = tf_agents.environments.ParallelPyEnvironment(
        [
            lambda itr_seed=seed + i: create_env_fn(itr_seed)
            for i in range(environment_batch_size)
        ]
    )

    batched_tf_env = tf_py_environment.TFPyEnvironment(parallel_tf_env)
