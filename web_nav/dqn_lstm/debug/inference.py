...
def load_model():
  ...
  q_net = DQNLSTM(
      ...
      state_spec=(),
      vocab_size=max_vocab_size if max_vocab_size is not None else
      tf_env.pyenv.envs[
...
def preprocess_observation(observation):
  ...
  for key in observation:
    observation[key] = tf.convert_to_tensor(observation[key],
                                            dtype=observation[key].dtype)

  time_step = ts.TimeStep(step_type=ts.StepType.FIRST, reward=0.0, discount=1.0,
...
def infer_once(model, observation):
  ...
  action = tf.nest.map_structure(lambda t: tf.squeeze(t, axis=0),
                                 action_step.action)
  return action
