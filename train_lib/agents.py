...
def _create_ppo_agent(
    ...
    **kwargs,
    ...
    if env_name in ENV_NAMES[BenchmarkDomain.CIRCUIT_TRAINING]:
        ...
        return create_circuit_ppo_agent(
...
def _create_sac_agent(
    ...
    **kwargs,
    ...
    return sac_agent.SacAgent(
        ...
        actor_network=actor_net,
        critic_network=critic_net,
        actor_optimizer=tf.keras.optimizers.Adam(learning_rate=lr, epsilon=1e-5),
        critic_optimizer=tf.keras.optimizers.Adam(learning_rate=lr, epsilon=1e-5),
        alpha_optimizer=tf.keras.optimizers.Adam(learning_rate=lr, epsilon=1e-5),
        target_update_tau=0.005,
        target_update_period=1,
        td_errors_loss_fn=tf.math.squared_difference,
        gamma=0.99,
