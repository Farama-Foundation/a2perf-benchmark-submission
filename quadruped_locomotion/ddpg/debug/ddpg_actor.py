...
def train(
  ...
  eval_env = gymnasium.make('QuadrupedLocomotion-v0',
                            motion_files=[motion_file_path], mode='test',
                            enable_rendering=visualize)
  ...
  save_iters = max(1, int(
    int_save_freq / (parallel_cores * timesteps_per_actorbatch)))
  eval_iters = max(1, int(
    int_eval_freq / (parallel_cores * timesteps_per_actorbatch)))

  print("save_iters:", save_iters)
  print(
    f'save_iters corresponds to {save_iters * parallel_cores * timesteps_per_actorbatch} environment steps')

  print("eval_iters:", eval_iters)
  print(
    f'eval_iters corresponds to {eval_iters * parallel_cores * timesteps_per_actorbatch} environment steps')
  ...
  if rank == 0:
    model.save(os.path.join(output_dir, "final_ddpg_policy"))
...
if __name__ == '__main__':
  ...
  arg_parser.add_argument("--int_eval_freq", dest="int_eval_freq", type=int,
                          default=0)
  arg_parser.add_argument("--optim_batchsize", dest="optim_batchsize", type=int,
                          default=0)
  arg_parser.add_argument("--timesteps_per_actorbatch",
                          dest="timesteps_per_actorbatch", type=int,
                          default=0)
