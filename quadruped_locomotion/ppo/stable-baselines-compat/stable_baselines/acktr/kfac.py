...
class KfacOptimizer:
    ...
    def compute_stats(self, loss_sampled, var_list=None):
        ...
        if KFAC_DEBUG:
            ...
            stats_updates[a_key] = tf.compat.v1.Print(stats_updates[a_key], [tf.convert_to_tensor('step:'), self.global_step,
    ...
    def apply_gradients_kfac(self, grads):
        ...
        if self._async_eigen_decomp:
            ...
            enqueue_op = tf.cond(
                tf.logical_and(tf.equal(tf.math.floormod(self.stats_step, self._kfac_update), tf.convert_to_tensor(
        ...
        with tf.control_dependencies([global_step_op]):
            ...
            with tf.control_dependencies(dependency_list):
                ...
                if not self._async_eigen_decomp:
                    ...
                    update_factor_ops = tf.cond(tf.logical_and(tf.equal(tf.math.floormod(self.stats_step, self._kfac_update),
