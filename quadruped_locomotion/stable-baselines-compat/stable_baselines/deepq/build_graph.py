        ...
        for var, perturbed_var in zip(all_vars, all_perturbed_vars):
            if param_noise_filter_func(perturbed_var):
                ...
                operation = tf.compat.v1.assign(perturbed_var,
                                      var + tf.random.normal(shape=tf.shape(var), mean=0.,
...
def build_train(q_func, ob_space, ac_space, optimizer, sess, grad_norm_clipping=None,
    ...
    with tf.compat.v1.variable_scope("loss", reuse=reuse):
        ...
        tf.compat.v1.summary.scalar("td_error", tf.reduce_mean(td_error))
    ...
    with tf.compat.v1.variable_scope("input_info", reuse=False):
        tf.compat.v1.summary.scalar('rewards', tf.reduce_mean(rew_t_ph))
        tf.compat.v1.summary.scalar('importance_weights', tf.reduce_mean(importance_weights_ph))
