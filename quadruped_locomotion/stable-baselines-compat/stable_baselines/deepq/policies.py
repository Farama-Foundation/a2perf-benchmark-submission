        ...
        with tf.compat.v1.variable_scope("model", reuse=reuse):
            ...
            if self.dueling:
                ...
                with tf.compat.v1.variable_scope("state_value"):
                    ...
                    # Compute the state score as a single output of the last fully connected layer
                    state_score = tf_layers.fully_connected(state_out, num_outputs=1, activation_fn=None)

                # Process the action scores to center them around their mean
                action_scores_mean = tf.reduce_mean(action_scores, axis=1)
                action_scores_centered = action_scores - tf.expand_dims(action_scores_mean, axis=1)

                # Combine the state score with the centered action scores to obtain the final Q-values
                q_out = state_score + action_scores_centered
            else:
                # If not using dueling, the Q-values are the action scores directly
                q_out = action_scores
