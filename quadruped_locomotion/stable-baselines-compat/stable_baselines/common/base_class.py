...
from stable_baselines.common.save_util import json_to_data
from stable_baselines.common.save_util import params_to_bytes
...
class BaseRLModel(ABC):
  ...
  def get_parameters(self):
    ...
    return_dictionary = OrderedDict((param.name, value) for param, value in
                                    zip(parameters, parameter_values))
  ...
  def _setup_load_operations(self):
    ...
    with self.graph.as_default():
      for param in loadable_parameters:
        placeholder = tf.compat.v1.placeholder(dtype=param.dtype,
                                               shape=param.shape)
        # param.name is unique (tensorflow variables have unique names)
        self._param_load_ops[param.name] = (
            placeholder, param.assign(placeholder))
      ...
      try:
        while True:
          ...
          if num_batches % print_frequency == 0:
            ...
            print(
                f"Epoch {epoch_idx + 1}, Batch {num_batches} - Current Avg Training Loss: {current_loss:.6f}")

      except tf.errors.OutOfRangeError:
        pass

      if num_batches > 0:
        train_loss /= num_batches  # Average loss per batch

      # Log and print the average training loss
      if self.verbose > 0:
        print(
...
class ActorCriticRLModel(BaseRLModel):
  ...
  def __init__(self, policy, env, _init_setup_model, verbose=0,
      ...
      requires_vec_env=False, policy_kwargs=None, seed=None,
      n_cpu_tf_sess=None):
  ...
  def action_probability(self, observation, state=None, mask=None, actions=None,
    ...
    if len(actions_proba) == 0:  # empty list means not implemented
      warnings.warn(
          "Warning: action probability is not implemented for {} action space. Returning None."
    ...
    if actions is not None:  # comparing the action distribution, to given actions
      ...
      if isinstance(self.action_space, gym.spaces.Discrete):
        ...
        actions = np.swapaxes(actions, 0,
                              1)  # swap axis for easier categorical split
        ...
        log_normalizer = n_elts / 2.0 * np.log(2 * np.pi) + np.sum(logstd,
                                                                   axis=1)

        # Diagonal Gaussian action probability, for every action
        logprob = -np.sum(np.square(actions - mean) / (2 * np.square(std)),
                          axis=1) - log_normalizer

      else:
        warnings.warn(
            "Warning: action_probability not implemented for {} actions space. Returning None."
            .format(type(self.action_space).__name__))
        return None
  ...
  def load(cls, load_path, env=None, custom_objects=None, **kwargs):
    ...
    model = cls(policy=data["policy"], env=None,
                _init_setup_model=False)  # pytype: disable=not-instantiable
...
class OffPolicyRLModel(BaseRLModel):
  ...
  def __init__(self, policy, env, replay_buffer=None, _init_setup_model=False,
      verbose=0, *,
      requires_vec_env=False, policy_base=None,
      policy_kwargs=None, seed=None, n_cpu_tf_sess=None):
    super(OffPolicyRLModel, self).__init__(policy, env, verbose=verbose,
                                           requires_vec_env=requires_vec_env,
                                           policy_base=policy_base,
                                           policy_kwargs=policy_kwargs,
                                           seed=seed,
...
class _UnvecWrapper(VecEnvWrapper):
  ...
  def compute_reward(self, achieved_goal, desired_goal, _info):
    return float(
        self.venv.env_method('compute_reward', achieved_goal, desired_goal,
                             _info)[0])
