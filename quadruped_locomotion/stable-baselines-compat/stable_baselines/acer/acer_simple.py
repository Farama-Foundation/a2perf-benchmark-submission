        ...
        with SetVerbosity(self.verbose):
            ...
            with self.graph.as_default():
                ...
                with tf.compat.v1.variable_scope("input_info", reuse=False):
                    tf.compat.v1.summary.scalar('rewards', tf.reduce_mean(self.reward_ph))
                    tf.compat.v1.summary.scalar('learning_rate', tf.reduce_mean(self.learning_rate))
                    tf.compat.v1.summary.scalar('advantage', tf.reduce_mean(adv))
                    tf.compat.v1.summary.scalar('action_probability', tf.reduce_mean(self.mu_ph))
